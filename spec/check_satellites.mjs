...
// --- the bundled transmitter file -------------------------------------------
// Generated by tools/build_transmitters.py from SatNOGS DB. Checked against the
// roster it was built from, because the two live in different files and the
// failure is silent: a satellite added to ROSTER without re-running the tool
// gets a pass page with no frequencies on it and no error anywhere.
const tx = JSON.parse(read('data/transmitters.json'));
check('the file names its source', /satnogs/i.test(tx.source), true);
check('and its licence, which is share-alike and must be stated', tx.license, 'CC BY-SA 4.0');

for (const entry of ROSTER) {
  const list = tx.satellites[String(entry.catalog)];
  check(`${entry.label} has transmitters`, Array.isArray(list) && list.length > 0, true);
  if (!Array.isArray(list)) continue;
  for (const t of list) {
    check(`${entry.label}: every transmitter is on a frequency`,
      Boolean(t.uplink || t.downlink), true);
    // Amateur satellites live between 10 m and the microwave bands. A value
    // outside that is a units error -- kilohertz where hertz was meant.
    for (const hz of [t.uplink, t.downlink]) {
      if (!hz) continue;
      check(`${entry.label}: ${hz} Hz is a plausible frequency`,
        hz > 2.8e7 && hz < 2.5e10, true);
    }
    check(`${entry.label}: invert is a boolean`, typeof t.invert, 'boolean');
  }
}

// Nothing outside the roster should be in the file: it is built from ROSTER, so
// a stray key means the tool was run against a different list.
for (const key of Object.keys(tx.satellites)) {
  check(`${key} is on the roster`, ROSTER.some((r) => String(r.catalog) === key), true);
}
